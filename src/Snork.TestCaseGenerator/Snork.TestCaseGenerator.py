...
class TextWrapper:
    ...
    def _munge_whitespace(self, text):
        ...
        """
        #z = len(text)
        #q = self._getHash(text)
        #self._writeFile("beforeExpandTabs.txt", text)
        if self.expand_tabs:
            text = text.expandtabs(self.tabsize)
        #self._writeFile("afterExpandTabs.txt", text)
        #r = self._getHash(text)
        #a = len(text)
        if self.replace_whitespace:
            text = text.translate(self.unicode_whitespace_trans)
        #s = self._getHash(text)
        #c = len(text)
        return text
...
if __name__ == "__main__":
    ...
    for func in functions:
        ...
        for placeholder in placeholders:
            for max_lines in maxlineoptions:
                for initial_indent in indents:
                    for subsequent_indent in indents:
                        for break_long_words in bools:
                            for tabsize in tabsizes:
                                for drop_whitespace in bools:
                                    for break_on_hyphens in bools:
                                        for replace_whitespace in bools:
                                            for fix_sentence_endings in bools:
                                                for expand_tabs in bools:
                                                    for width in range(15, 75, 30):
                                                        text=textoptions[random.randint(0,len(textoptions)-1)]
                                                        data={}
                                                        data["function"] = func
                                                        data["placeholder"]=placeholder
                                                        data["max_lines"]=max_lines
                                                        data["initial_indent"] = initial_indent
                                                        data["subsequent_indent"] = subsequent_indent
                                                        data["break_long_words"] = break_long_words
                                                        data["tabsize"] = tabsize
                                                        data["drop_whitespace"]=drop_whitespace
                                                        data["break_on_hyphens"]=break_on_hyphens
                                                        data["width"]=width
                                                        data["text"]=text
                                                        data["expand_tabs"]=expand_tabs
                                                        data["replace_whitespace"]=replace_whitespace
                                                        data["fix_sentence_endings"]=fix_sentence_endings

                                                        encoded = json.dumps(data).encode();
                                                        hash_obj = hashlib.sha1(encoded);
                                                        data["id"] = hash_obj.hexdigest();
                                                        if len(singleCase) ==0 or data["id"]==singleCase:
                                                            if func=="wrap":
                                                                data["expected"]=wrap(text,width,expand_tabs=expand_tabs,placeholder=placeholder, replace_whitespace=replace_whitespace,max_lines=max_lines, fix_sentence_endings=fix_sentence_endings, break_on_hyphens=break_on_hyphens,drop_whitespace=drop_whitespace, tabsize = tabsize, break_long_words = break_long_words, initial_indent = initial_indent, subsequent_indent = subsequent_indent)
                                                            elif func == "fill":
